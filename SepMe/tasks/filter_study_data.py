...
def filter_study_data(path, col_a, col_m, filter_out, max_diff):
    with mlflow.start_run(run_name="filter_study_data") as mlrun:
        ...
        df = df[(~df[col_a].isin(filter_out)) & (~df[col_m].isin(filter_out)) & (np.abs(df[col_a] - df[col_m]) <= max_diff)]
        logger.log_metric('df_rows', len(df[(~df[col_a].isin(filter_out)) & (~df[col_m].isin(filter_out)) & (np.abs(df[col_a] - df[col_m]) <= max_diff)]))
