...
def calculate_dsc(sample_df, class_names):
    ...
    for class_value in set(sample_df["class"]):
        centroids[class_value] = tuple(
            np.mean(sample_df[sample_df["class"] == class_value])[0:2]
        )
        centroids_list.append(
            tuple(np.mean(sample_df[sample_df["class"] == class_value])[0:2])
        )
...
def dsc(data_path, folder_path, class_names):
    with mlflow.start_run(run_name="filter_study_data") as mlrun:
        ...
        df[["fileName", "classNum", "DSC_mlflow", "DSC"]].to_csv(
            "data/results/df.csv", index=False
        )
