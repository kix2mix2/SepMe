...
def plot_colored_circles(ax, df, circles, class_name='class'):
    for i, row in df.iterrows():
        ax.add_patch(PolygonPatch(circles[i], fc = sns.color_palette("colorblind")[int(row[class_name])], ec = 'none',
                                  alpha = 1))
...
def get_dimred_data(df, input_folder, save_folder, fig_folder):
    ...
    for i, nn in enumerate(names):
        ...
        try:
            ...
            ddf, circles = preprocess_df(data, [dim_x, dim_y], size = 0.5, occlusion = 0.1, save = file_name,
                                         sort = True)
