    ...
    if os.path.exists(graph_path):
        ...
        if "mst" in config:
            if graph_types["del"] is not None:
                ...
                graph_types["mst"] = add_node_attr(
                    get_mst(graph_types["del"]), df
                )

        if "knn" in config:
            graph_types["knn"] = {}
            for param in config["knn"]:
                graph_types["knn"][param] = add_node_attr(
                    get_knntree(df, param), df
                )

        if "kncg" in config:
            graph_types["kncg"] = {}
            for param in config["kncg"]:
                graph_types["kncg"][param] = add_node_attr(
                    get_kncg(df, param), df
                )

        if "gong" in config:
            graph_types["gong"] = {}
            for param in config["gong"]:
                graph_types["gong"][param] = add_node_attr(
                    get_gong(df, param), df
                )
        ...
        if "rng" in config:
            ...
            if graph_types["mst"] is None:
                graph_types["mst"] = graph_types["mst"] = get_mst(
                    graph_types["del"]
                )
            graph_types["rng"] = {}
            graph_types["rng"] = add_node_attr(
                get_rng(df, graph_types["del"], graph_types["mst"]), df
            )

        if "bt" in config:
            graph_types["bt"] = {}
            for param in config["bt"]:
                graph_types["bt"][param] = add_node_attr(
                    get_balltree(df, param), df
                )
        ...
        if "cbsg" in config:
            ...
            for param in config["cbsg"]:
                graph_types["cbsg"][param] = add_node_attr(
                    get_CBSG(df, param), df
                )
...
def calculate_purities(df, graph, purities):
    ...
    neighbour_purity_list = list(
        set(["cp", "ce", "mv"]).intersection(set(purities["type"]))
    )
    ...
    for pessimism in purities["pessimism"]:
        purity_dict.update(
            total_neighbour_purity(
                df, graph, None, neighbour_purity_list, pessimism
            )
        )
